...
const utils = {
    ...
    async getAuthorization(platform) {
        ...
        try {
            ...
            // support no defaulttenant
            const tenanthost = TenantName && TenantName.toLowerCase() === 'defaulttenant' ? '' : `${TenantName}.`;
            const userBaseURL = `${protocol}//${tenanthost}${nuimsDomain}`.replace(/\/$/, '');
